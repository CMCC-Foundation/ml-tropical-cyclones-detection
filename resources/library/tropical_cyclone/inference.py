...
def load_ibtracs(ibtracs_src, lat_range=None, lon_range=None):
    ...
    ibtracs = ibtracs[(ibtracs['LAT'] >= lat_range[0]) & (ibtracs['LAT'] <= lat_range[1]) & (ibtracs['LON'] >= lon_range[0]) & (ibtracs['LON'] <= lon_range[1])]
...
def get_observed_tracks(observations):
    ...
    # sort tracks
    observed_tracks = observed_tracks.sort_values('TRACK_ID').sort_values('ISO_TIME').reset_index(drop=True)
...
def iqr_localization(y_pred, n_models):
    ...
    y_pred_filtered = np.where((y_pred > lower_bound) & (y_pred < upper_bound), y_pred, np.nan)
    # compute mean over the entire dataset
    mu = np.round(np.nanmean(np.where(np.repeat((~np.isnan(y_pred_filtered)).astype(np.int8).min(axis=2)[:,:,np.newaxis], repeats=2, axis=2) > 0, y_pred_filtered, np.nan), axis=1), 0)
    # compute mean over the entire dataset
    std = np.nanstd(np.where(np.repeat((~np.isnan(y_pred_filtered)).astype(np.int8).min(axis=2)[:,:,np.newaxis], repeats=2, axis=2) > 0, y_pred_filtered, np.nan), axis=1)
    return mu, std
...
def load_trained_model(model_dir, device='cpu'):
    ...
    model_weights_file = sorted(glob.glob(os.path.join(model_dir, 'checkpoints','*.ckpt')))[-1]
...
class Inference():
    ...
    def tracking(self, detections, max_distance=400.0, min_track_count=12):
        # initialize tracking dataframe
        detected_tracks = init_track_dataframe(detections)
        # apply tracking scheme to detections
        detected_tracks = tracking_algorithm(track_df=detected_tracks, max_distance=max_distance, min_track_count=min_track_count)
...
class EnsembleModelInference(Inference):
    ...
    def _retrieve_ensemble_predictions(self, models_predictions, epsilon, patch_size, n_consensus, label_no_cyclone, time, rows, cols):
        ...
        n_tc_pred = np.repeat((y_pred > 0).astype(np.int8).min(axis=2).sum(axis=1)[:,np.newaxis], repeats=2, axis=1)
        # localize points
    ...
    def predict(self, ds, n_consensus, label_no_cyclone=-1.0, patch_size=40, epsilon=0):
        ...
        patch_ds = ds.coarsen({'lat':patch_size, 'lon':patch_size}, boundary="trim").construct({'lon':("cols", "lon_range"), 'lat':("rows", "lat_range")})
        # get dataloader
        data_loader = prepare_dataloader(patch_ds=patch_ds, patch_size=patch_size, scaler=self.scaler, drivers=self.drivers, time=time, rows=rows, cols=cols, channels=channels, batch_size=4096)
        # predict with the models
        models_predictions = predict_with_models(models=self.models, data_loader=data_loader, device=self.device)
        # get the patch ds filled with predicted tc coordinates
        y_pred, _, _ = self._retrieve_ensemble_predictions(models_predictions, epsilon, patch_size, n_consensus, label_no_cyclone, time, rows, cols)
        # get predicted cyclone coordinates
