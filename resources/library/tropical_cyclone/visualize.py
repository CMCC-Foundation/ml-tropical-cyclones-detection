...
def plot_tracks(
    det_tracks, obs_tracks, lat_range=(0, 70), lon_range=(100, 320), outfile=None
):
    # set map extent
    central_longitude = lon_range[1] - lon_range[0]
    ...
    for i, id in enumerate(obs_tracks["TRACK_ID"].unique()):
        ax.plot(
            obs_tracks[obs_tracks["TRACK_ID"] == id]["LON"],
            obs_tracks[obs_tracks["TRACK_ID"] == id]["LAT"],
            alpha=0.2,
            transform=transform,
            color="blue",
        )
        ax.scatter(
            ...
            transform=transform,
            color="blue",
            label=f'Observed Tracks (#{len(obs_tracks["TRACK_ID"].unique())})'
            if i == 0
            else None,
        )
    for i, id in enumerate(det_tracks["TRACK_ID"].unique()):
        ax.plot(
            det_tracks[det_tracks["TRACK_ID"] == id]["LON"],
            det_tracks[det_tracks["TRACK_ID"] == id]["LAT"],
            alpha=0.2,
            transform=transform,
            color="red",
        )
        ax.scatter(
            ...
            transform=transform,
            color="red",
            label=f'Detected Tracks (#{len(det_tracks["TRACK_ID"].unique())})'
            if i == 0
            else None,
        )
