...
def get_nocyclones_patches(dataset, cyclone_patch_ids, patch_size=40):
    ...
    # get all coordinates
    all_coords = [
        coords for coords in list(itertools.product(patch_row_idx, patch_col_idx))
    ]
...
def get_all_adjacent_patches(dataset, patch_cyclone_ids, patch_size=40):
    ...
    for patch_id in patch_cyclone_ids:
        i, j = patch_id
        tmp_all_adjacent_ids += [
            a for a in itertools.product([i - 1, i, i + 1], [j - 1, j, j + 1])
        ]
    ...
    all_adjacent_ids = all_adjacent_ids.difference(
        set(list(map(tuple, patch_cyclone_ids)))
    )
    return all_adjacent_ids


def get_nearest_adjacent_patches(
    dataset, patch_cyclone_ids, patch_cyclone_positions, patch_size=40
):
    """
    ...
    for patch_id, cyclone_position in zip(patch_cyclone_ids, patch_cyclone_positions):
        ...
        if is_first_quadrant(y, x, half_patch_size):
            ...
            nearest_patch_ids.add(
                (i - 1, j + 1)
            ) if i - 1 >= 0 and j + 1 < col_blocks else None
...
def get_random_patches(dataset, patch_cyclone_ids, patch_size=40):
    ...
    no_tc_coords_candidates = [
        coords
        for coords in list(itertools.product(patch_row_idx, patch_col_idx))
        if coords not in inv_coords
    ]
