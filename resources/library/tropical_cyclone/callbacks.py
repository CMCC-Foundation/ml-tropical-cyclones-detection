...
class DiscordLog(Callback):
    def __init__(self, model_name: str = 'default', webhook_url: str = None, benchmark_csv: str = None, msg_every_n_epochs: int = 1, plot_every_n_epochs: int = 5) -> None:
        super().__init__()
    ...
    def __log_grad_plot(self, trainer: Trainer):
        ...
        plt.ylabel('Norm')
        plt.legend()
        outfile = os.path.join(os.path.dirname(self.benchmark_csv), f'plot_grad_{trainer.current_epoch}.png')
        plt.savefig(outfile, dpi=200)
        # prepare a payload to send the image
