...
def get_tropical_cyclone_positions(
    ds: xr.Dataset, georef: pd.DataFrame, sigma: int = 10
) -> xr.Dataset:
    """
    ...
    for t, iso_time in enumerate(gr["ISO_TIME"].unique()):
        ...
        gaussian_map[t, :, :] = (
            gaussian_map[t, :, :] - gaussian_map[t, :, :].min()
        ) / (gaussian_map[t, :, :].max() - gaussian_map[t, :, :].min())
...
def init_track_dataframe(detections: pd.DataFrame):
    ...
    tracks = tracks[(~np.isnan(tracks["LAT"])) & (~np.isnan(tracks["LON"]))][
        ["ISO_TIME", "LAT", "LON", "WS"]
...
def tracking_algorithm(detected_tracks, max_distance=400.0, min_track_count=12):
    ...
    for t in range(len(iso_times) - 1):
        ...
        # get next detections
        next_detections = detected_tracks[
            detected_tracks["ISO_TIME"] == iso_times[t + 1]
        ]

        # check of 6-hour timelag
        time_delta_days = (
            pd.to_datetime(iso_times[t + 1]) - pd.to_datetime(iso_times[t])
        ).days
        time_delta_hr = (
            pd.to_datetime(iso_times[t + 1]) - pd.to_datetime(iso_times[t])
        ).seconds / 3600
        ...
        for i, detection in cur_detections.iterrows():
            # prepare a track id
            new_track_id = (
                str(iso_times[t]).replace(" ", "-").replace("-", "").split(":")[0]
                + f"_{k}"
            )
            k += 1
            ...
            if detection["TRACK_ID"] == "":
                ...
                detected_tracks.iloc[
                    i, detected_tracks.columns.get_loc("TRACK_ID")
                ] = new_track_id
        ...
        merge["HAVERSINE"] = haversine_vector(
            array1=merge[["LAT_x", "LON_x"]].to_numpy().astype(np.float32),
            array2=merge[["LAT_y", "LON_y"]].to_numpy(),
            normalize=True,
        )
        ...
        while True:
            ...
            for i, row in merge.iterrows():
                ...
                if row["TRACK_ID_y"] == "":
                    detected_tracks.iloc[
                        detected_tracks[
                            (detected_tracks["LAT"] == row["LAT_y"])
                            & (detected_tracks["LON"] == row["LON_y"])
                            & (detected_tracks["ISO_TIME"] == row["ISO_TIME_y"])
                        ].index,
                        detected_tracks.columns.get_loc("TRACK_ID"),
                    ] = row["TRACK_ID_x"]
                    detected_tracks.iloc[
                        detected_tracks[
                            (detected_tracks["LAT"] == row["LAT_x"])
                            & (detected_tracks["LON"] == row["LON_x"])
                            & (detected_tracks["ISO_TIME"] == row["ISO_TIME_x"])
                        ].index,
                        detected_tracks.columns.get_loc("HAVERSINE"),
                    ] = row["HAVERSINE"]
                else:
                    continue
                ...
                merge = merge[
                    ~(
                        (merge["ISO_TIME_x"] == row["ISO_TIME_x"])
                        & (merge["LAT_x"] == row["LAT_x"])
                        & (merge["LON_x"] == row["LON_x"])
                    )
                ]
                merge = merge[
                    ~(
                        (merge["ISO_TIME_y"] == row["ISO_TIME_y"])
                        & (merge["LAT_y"] == row["LAT_y"])
                        & (merge["LON_y"] == row["LON_y"])
                    )
                ]
                break
    ...
    for i, detection in cur_detections.iterrows():
        # prepare a track id
        new_track_id = (
            str(iso_times[-1]).replace(" ", "-").replace("-", "").split(":")[0]
            + f"_{k}"
        )
        k += 1
        # if the detection is new, it will have a new track id
    ...
    detected_tracks = (
        detected_tracks.groupby("TRACK_ID")
        .filter(lambda x: len(x) >= min_track_count)
        .reset_index(drop=True)
    )
    return detected_tracks


def paper_tracking_algorithm(
    detected_tracks, max_distance=400.0, min_track_count=12, min_wind_speed=17.0
):
    iso_times = sorted(detected_tracks["ISO_TIME"].unique())
    ...
    for t, iso_time in enumerate(iso_times):
        ...
        for i, detection in detected_tracks[
            detected_tracks["ISO_TIME"] == iso_time
        ].iterrows():
            # prepare a track id
            new_track_id = (
                str(iso_time).replace(" ", "-").replace("-", "").split(":")[0] + f"_{k}"
            )
            k += 1
            ...
            try:
                # check of 6-hour timelag
                time_delta_days = (
                    pd.to_datetime(iso_times[t + 1]) - pd.to_datetime(iso_time)
                ).days
                time_delta_hr = (
                    pd.to_datetime(iso_times[t + 1]) - pd.to_datetime(iso_time)
                ).seconds / 3600
            ...
            next_detections = detected_tracks[
                detected_tracks["ISO_TIME"] == next_iso_time
            ]
            ...
            if min_dist <= max_distance:
                ...
                detected_tracks.iloc[
                    i, detected_tracks.columns.get_loc("HAVERSINE")
                ] = min_dist
                ...
                if min_dist_track_id == "":
                    ...
                    detected_tracks.iloc[
                        min_arg_idx, detected_tracks.columns.get_loc("TRACK_ID")
                    ] = track_id
                else:
                    new_detection = {
                    ...
                    detected_tracks = pd.concat(
                        [detected_tracks, pd.DataFrame(new_detection)]
                    )

    clean_pred_track_with_wind_df = pd.DataFrame(
        {
            "ISO_TIME": [],
            "LAT": [],
            "LON": [],
    ...
    clean_pred_track_without_wind_df = pd.DataFrame(
        {
            "ISO_TIME": [],
            "LAT": [],
            "LON": [],
    ...
    for track_id in detected_tracks["TRACK_ID"].unique():
        ...
        if len(wind_track_id_df) >= min_track_count:
            clean_pred_track_with_wind_df = pd.concat(
                [clean_pred_track_with_wind_df, track_id_df]
            )
    ...
    for track_id in detected_tracks["TRACK_ID"].unique():
        ...
        if len(track_id_df) >= min_track_count:
            clean_pred_track_without_wind_df = pd.concat(
                [clean_pred_track_without_wind_df, track_id_df]
            )
...
def track_matching(
    ...
    complete_match_df = pd.DataFrame(
        data={"ISO_TIME": [], "OBS_TRACK_ID": [], "DET_TRACK_ID": []}
    )
    ...
    for detected_track_id in detected_tracks["TRACK_ID"].unique():
        ...
        for i, detected_location in detected_tracks[
            detected_tracks["TRACK_ID"] == detected_track_id
        ].iterrows():
            # get all the observed tracks in that iso time
            selected_observed_points = observed_tracks[
                observed_tracks["ISO_TIME"] == detected_location["ISO_TIME"]
            ].copy()

            # no matches are found
            if not len(selected_observed_points):
                complete_match_df = pd.concat(
                    [
                        complete_match_df,
                        pd.DataFrame(
                            data={
                                "ISO_TIME": [detected_location["ISO_TIME"]],
                                "OBS_TRACK_ID": [[]],
                                "DET_TRACK_ID": [detected_track_id],
                            }
                        ),
                    ]
                )
            ...
            OD_paired = selected_observed_points[
                selected_observed_points["OD_DIST"] <= max_track_distance
            ]
            ...
            complete_match_df = pd.concat(
                [
                    ...
                    pd.DataFrame(
                        data={
                            "ISO_TIME": [detected_location["ISO_TIME"]],
                            "OBS_TRACK_ID": [list(OD_paired["TRACK_ID"].unique())],
                            "DET_TRACK_ID": [detected_track_id],
                        }
                    ),
                ]
    ...
    for detected_track_id in detected_tracks["TRACK_ID"].unique():
        ...
        obs_track_ids_unique = list(
            set(
                [
                    tti
                    for ttis in complete_match_df[
                        complete_match_df["DET_TRACK_ID"] == detected_track_id
                    ]["OBS_TRACK_ID"].to_numpy()
                    for tti in ttis
                ]
            )
        )
        ...
        for obs_track_ids in complete_match_df[
            complete_match_df["DET_TRACK_ID"] == detected_track_id
        ]["OBS_TRACK_ID"].to_numpy():
        ...
        if not len(obs_track_id_count.keys()):
            match_df = pd.concat(
                [
                    match_df,
                    pd.DataFrame(
                        data={"OBS_TRACK_ID": [""], "DET_TRACK_ID": [detected_track_id]}
                    ),
                ]
            )
            continue
        ...
        match_df = pd.concat(
            [
                ...
                pd.DataFrame(
                    data={
                        "OBS_TRACK_ID": [
                            max(obs_track_id_count, key=obs_track_id_count.get)
                        ],
                        "DET_TRACK_ID": [detected_track_id],
                    }
                ),
    ...
    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
    for obs_track_id in match_df[match_df["OBS_TRACK_ID"] != ""][
        "OBS_TRACK_ID"
    ].unique():
        # get all the true tracks in that iso time
        select_det_track_ids = match_df[match_df["OBS_TRACK_ID"] == obs_track_id][
            "DET_TRACK_ID"
        ].to_numpy()
        ...
        if len(select_det_track_ids) > 1:
            ...
            for detected_track_id in select_det_track_ids[1:]:
                ...
                match_df = match_df[
                    ~(
                        (match_df["OBS_TRACK_ID"] == obs_track_id)
                        & (match_df["DET_TRACK_ID"] == detected_track_id)
                    )
                ]
    ...
    missed_track_ids = set(observed_tracks["TRACK_ID"].unique()).difference(
        set(match_df["OBS_TRACK_ID"].unique())
    )
    ...
    for track_id in missed_track_ids:
        ...
        match_df = pd.concat(
            [
                match_df,
                pd.DataFrame(data={"OBS_TRACK_ID": [track_id], "DET_TRACK_ID": [""]}),
            ]
        )
