...
def get_attn_mask_3d(input_resolution, win_size, shift_size, suppl_attn_mask=None):
    if any(i > 0 for i in shift_size):
        ...
        for d in (
            ...
            for h in (
                ...
                for w in (
                    ...
                    img_mask[:, d, h, w, :] = cnt
                    cnt += 1
        mask_windows = window_partition_3d(
            img_mask, win_size
        )  # nW, ws[0]*ws[1]*ws[2], 1
        ...
        attn_mask = (
            attn_mask + suppl_attn_mask if suppl_attn_mask is not None else attn_mask
        )
    else:
        # attn_mask = None
...
class SpaceTimeConv(nn.Module):
    def __init__(
        ...
        self.spatial_conv = nn.Conv2d(
            in_features,
            hidden_features,
            kernel_size=space_kernel_size,
            padding="same",
            bias=bias,
        )
        self.temporal_conv = temporal_conv(
            hidden_features, out_features, kernel_size=time_kernel_size, bias=bias
        )
...
class PatchEmbed(nn.Module):
    def __init__(
        ...
        self.space_time_conv = SpaceTimeConv(
...
class Downsample(nn.Module):
    ...
    def __init__(
        ...
        self.reduction = nn.Linear(
            in_features=4 * dim, out_features=dim_factor * dim, bias=False
        )
    ...
    def _merge(self, x: torch.Tensor, res: tuple[int, int, int]):
        ...
        _, T, H, W, _ = x.shape
        x = rearrange(
            x, "B T (H h) (W w) C -> B T (H W) (h w C)", H=H // 2, h=2, W=W // 2, w=2
        )
        return x, (T, H // 2, W // 2)
...
class WindowAttention3D_v1(nn.Module):
    ...
    def __init__(
        ...
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros(
                (2 * win_size[0] - 1) * (2 * win_size[1] - 1) * (2 * win_size[2] - 1),
                num_heads,
            )
...
class WindowAttention3D_v2(nn.Module):
    ...
    def __init__(
        ...
        relative_coords_t = torch.arange(
            -(self.win_size[0] - 1), self.win_size[0], dtype=torch.float32
        )
        relative_coords_h = torch.arange(
            -(self.win_size[1] - 1), self.win_size[1], dtype=torch.float32
        )
        relative_coords_w = torch.arange(
            -(self.win_size[2] - 1), self.win_size[2], dtype=torch.float32
        )
        relative_coords_table = (
            torch.stack(
                torch.meshgrid(
                    [relative_coords_t, relative_coords_h, relative_coords_w]
                )
            )
            .permute(1, 2, 3, 0)
            .contiguous()
    ...
    def forward(
        ...
        relative_position_bias = relative_position_bias_table[
            ...
            self.win_size[0] * self.win_size[1] * self.win_size[2],
            self.win_size[0] * self.win_size[1] * self.win_size[2],
            -1,
        )  # Wh*Ww,Wh*Ww,nH
        relative_position_bias = relative_position_bias.permute(
    ...
    def extra_repr(self) -> str:
        return (
            f"dim={self.dim}, window_size={self.win_size}, "
...
class SwinTransformerBlock3D_v1(nn.Module):
    ...
    def __init__(
        ...
        assert (
            0 <= self.shift_size[0] < self.win_size[0]
        ), "shift_size must in 0-window_size"
        assert (
            0 <= self.shift_size[1] < self.win_size[1]
        ), "shift_size must in 0-window_size"
        assert (
            0 <= self.shift_size[2] < self.win_size[2]
        ), "shift_size must in 0-window_size"
    ...
    def forward(
        self, x: torch.Tensor, res: tuple[int, int, int], out_attn: bool = False
    ):
        _, H, W = res
        B, T, L, C = x.shape
        # if window size is larger than input resolution, we don't partition windows
        if (
            res[0] <= self.win_size[0]
            and res[1] <= self.win_size[1]
            and res[2] <= self.win_size[2]
        ):
            shift_size = (0, 0, 0)
            win_size = res
        else:
...
class SwinTransformerBlock3D_v2(nn.Module):
    ...
    def forward(
        ...
        x_windows = x_windows.view(
            -1, win_size[0] * win_size[1] * win_size[2], C
        )  # nW*B, window_size*window_size, C

        # W-MSA/SW-MSA
        attn_mask = get_attn_mask_3d(res, tuple(win_size), shift_size, None)
        attn_windows, attn = self.attn(
            x_windows, mask=attn_mask, out_attn=out_attn
        )  # nW*B, window_size*window_size, C
    ...
    def extra_repr(self) -> str:
        return (
            f"dim={self.channels}, num_heads={self.num_heads}, "
...
class BasicLayer(nn.Module):
    def __init__(
        ...
        shift_sizes = [
            (0, 0, 0)
            if (i % 2 == 0)
            else (win_size[0] // 2, win_size[1] // 2, win_size[2] // 2)
