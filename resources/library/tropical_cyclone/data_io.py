...
from tropical_cyclone.patch_proc import get_all_adjacent_patches, get_random_patches, get_nearest_adjacent_patches
...
class InterTwinDatasetWriter():
    ...
    def process_year(self,
        ...
        if os.path.exists(tc_dst) or os.path.exists(no_tc_dst) or os.path.exists(nr_dst) or os.path.exists(rn_dst):
            logging.info(f'   year already processed, skipping')
            return
        # define empty data
        tc_X = np.empty(shape=(0, len(self.patch_vars), self.patch_size, self.patch_size), dtype=self.dtype)
        tc_Y = np.empty(shape=(0, len(self.coo_vars), 2), dtype=self.dtype)
        no_tc_X = np.empty(shape=(0, len(self.patch_vars), self.patch_size, self.patch_size), dtype=self.dtype)
        ...
        nr_X = np.empty(shape=(0, len(self.patch_vars), self.patch_size, self.patch_size), dtype=self.dtype)
        ...
        for _, filepath in zip(tqdm(filepaths), filepaths):
            ...
            X_data = np.transpose(patch_ds.isel(time=0)[self.patch_vars].to_array().data, axes=(1,3,0,2,4)) # rows x cols x 40 x 40 x vars
            Y_data = np.transpose(patch_ds.isel(time=0)[self.coo_vars].to_array().data, axes=(1,2,0,3))
            # update datasets
            tc_X, tc_Y = self.__update_dataset(tc_X, tc_Y, X_data, Y_data, tc_ids)
            if is_test == True:
                no_tc_X, no_tc_Y = self.__update_dataset(no_tc_X, no_tc_Y, X_data, Y_data, no_tc_ids)
            # if is_test == False:
    ...
    def process_years_multiproc(self, 
            dst_dir, 
            years,
            rank,
            num_processes,
    ...
    def merge_zarr_files(self, 
            dst_dir: str, 
            pathname: str
        ):
        # create empty numpy datasets for X and Y
        X = np.empty(shape=(0, len(self.patch_vars), self.patch_size, self.patch_size), dtype=self.dtype)
    ...
    def __get_patch_ids(self, 
        ...
        # get all patch ids
        all_patch_ids = [coords for coords in list(itertools.product([i for i in range(i_bound)], [i for i in range(j_bound)]))]
        ...
        no_tc_ids = sorted(list(set(list(all_patch_ids)).difference(set(list(map(tuple, tc_ids))))))
        # get intra-patch cyclone positions
        patch_cyclone_positions = (georef[['YLAT','XLON']] % self.patch_size).to_numpy()
        ...
        nr_ids = get_nearest_adjacent_patches(dataset=ds, patch_cyclone_ids=tc_ids, patch_cyclone_positions=patch_cyclone_positions)
        return np.array(tc_ids), np.asarray(no_tc_ids), np.array(list(aa_ids)), np.array(list(nr_ids)), np.array(list(rn_ids))

    def __fill_patch_ds_with_tc_info(self, 
            patch_ds: xr.Dataset, 
            tc_ids: List, 
            georef: pd.DataFrame
        ):
        ...
        real_cyclone_data = np.full(shape=(patch_ds.time.data.shape[0], patch_ds.rows.data.shape[0], patch_ds.cols.data.shape[0], 2), fill_value=self.label_no_cyclone, dtype=self.dtype)
        rounded_cyclone_data = np.full(shape=(patch_ds.time.data.shape[0], patch_ds.rows.data.shape[0], patch_ds.cols.data.shape[0], 2), fill_value=self.label_no_cyclone, dtype=self.dtype)
        global_cyclone_data = np.full(shape=(patch_ds.time.data.shape[0], patch_ds.rows.data.shape[0], patch_ds.cols.data.shape[0], 2), fill_value=self.label_no_cyclone, dtype=self.dtype)
        patch_cyclone_data = np.full(shape=(patch_ds.time.data.shape[0], patch_ds.rows.data.shape[0], patch_ds.cols.data.shape[0], 2), fill_value=self.label_no_cyclone, dtype=self.dtype)
        # for each rowcol coordinate
        for tc_id, pt_rowcol, rcy_coo, gl_rowcol, cy_coo in zip(tc_ids, pcy_rowcol, rcy_coords, gcy_rowcol, cy_coords):
            i,j = tc_id
        ...
        patch_ds = patch_ds.assign({
            'real_cyclone': (('time', 'rows', 'cols', 'coordinate'), real_cyclone_data),
            'rounded_cyclone': (('time', 'rows', 'cols', 'coordinate'), rounded_cyclone_data),
            'global_cyclone': (('time', 'rows', 'cols', 'coordinate'), global_cyclone_data),
            'patch_cyclone': (('time', 'rows', 'cols', 'coordinate'), patch_cyclone_data),
        })
        return patch_ds
