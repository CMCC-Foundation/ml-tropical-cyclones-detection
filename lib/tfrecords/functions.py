...
def get_tensor_decoding_fn(shape, drv_vars=[], coo_vars=None, msk_var=None, dtype=tf.float32):
    ...
    def tensor_decoding_fn(serialized_data):
        ...
        # get x raw data
        Xdrv = tf.stack([ tf.ensure_shape(tf.io.parse_tensor(serialized=parsed_data[var], out_type=dtype), shape=shape)for var in drv_vars ], axis=-1)
        data.append(Xdrv)
        
        # if coordinate vars are provided
        if coo_vars:
            if len(coo_vars) == 1:
                Ycoo = tf.ensure_shape(tf.io.parse_tensor(serialized=parsed_data[coo_vars[0]], out_type=dtype), shape=(2,))
            else:
                Ycoo = tf.stack([ tf.ensure_shape(tf.io.parse_tensor(serialized=parsed_data[var], out_type=dtype), shape=(2)) for var in coo_vars ], axis=-1)
            data.append(Ycoo)
        
        # if mask var is provided
        if msk_var:
            Ymsk = tf.expand_dims(tf.ensure_shape(tf.io.parse_tensor(serialized=parsed_data[msk_var], out_type=dtype), shape=shape), axis=-1)
            data.append(Ymsk)
...
def get_scale_target_fn(label_no_cyclone, patch_size):
    ...
    def scale_target_fn(data):
        """Function to scale the target according to the provided label_no_cyclone"""
        x,y = data
        # scale y
...
def read_tfrecord_as_tensor(filenames, shape, drv_vars, coo_vars, msk_var):
    ...
    tensor_decoding_fn = get_tensor_decoding_fn(shape, drv_vars=drv_vars, coo_vars=coo_vars, msk_var=msk_var)
