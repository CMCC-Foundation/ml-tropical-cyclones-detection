...
if config.run.use_case == "cnn":
    ...
    for i in range(rank, len(test_years), size):
        ...
        ds, dates = inference.load_dataset(
            dataset_dir=dataset_dir, drivers=drivers, year=year, is_cmip6=False
        )
    ...
    for i in range(rank, len(test_years), size):
        ...
        dataset = tc.dataset.TCGraphDatasetInference(
            src=dataset_dir,
            year=year,
        ...
        loader = GraphDataLoader(
            dataset, batch_size=config.train.batch_size, shuffle=False, drop_last=False
        )
